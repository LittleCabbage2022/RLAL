# src/finetune.py
# nohup python -u -m src.finetune --config configs/full.yaml --checkpoint_dir checkpoints/resnet56_cifar10_full >finetune.log 2>&1 &
# python -m src.finetune --config configs/full.yaml --checkpoint_dir checkpoints/resnet56_cifar10_full
...
def finetune(config_path, checkpoint_dir):
    ...
    # 4. 加载 Stage 1 的权重 (可选，但推荐，作为良好的初始化)
    # 如果你想完全 Train from Scratch，可以注释掉这一段
    stage1_weights_path = os.path.join(checkpoint_dir, 'student_model.pth')
    if os.path.exists(stage1_weights_path):
        print(f">> Loading Stage 1 weights from {stage1_weights_path}")
        model.base.load_state_dict(torch.load(stage1_weights_path, map_location=device))
    else:
        print(">> Warning: Stage 1 weights not found. Training from random initialization.")

    # 5. 加载 Best Policy 并生成 Mask
    # 必须确保 best_policy.json 在 checkpoint_dir 下
    model.best_policy_path = os.path.join(checkpoint_dir, 'best_policy.json')
    masks = model.get_masks_from_best_policy()
    
    if masks is None:
        raise FileNotFoundError(f"Could not load best_policy.json from {checkpoint_dir}")
    
    print(">> Pruning Mask loaded. Applying initial pruning...")
    model.apply_hard_prune(masks) # 初始应用一次
    ...
    for epoch in range(finetune_epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            # 每次更新后重新应用 Mask，防止梯度更新让被剪掉的权重“复活”
            model.apply_hard_prune(masks)
